...
for (dataset in datasets) {
  ...
  mutate(cell_type = case_when(posterior_probability_sensitive > min_posterior_probability_sensitive ~ 'sensitive',
                               TRUE ~ 'resistant')) #%>%
  #mutate(cell_type = case_when(posterior_probability_sensitive >= .99 ~ 'sensitive',
#                               posterior_probability_sensitive <= .01 ~ 'resistant')) 
  }
...
for (drug_for_biomarkers in all_drugs) {
  ...
  for (study in names(rez)) {
    ...
    #(depVar in c('auc_recomputed','posterior_probability_sensitive')) {
         #c('waterfall','naive','auc_recomputed','posterior_probability_sensitive','cell_type')) {
      if (depVar == 'waterfall') { 
...
for (drug_for_biomarkers in all_drugs) {
  ...
  for (depVar in c('auc_recomputed','posterior_probability_sensitive', 'cell_type')) {
       #c('naive','auc_recomputed','posterior_probability_sensitive','cell_type')) {
    # gather expression data
    expr_data <- ourData %>% 
      select(-drug, -cell, 
             -auc_recomputed, -naive, -cell_type, 
