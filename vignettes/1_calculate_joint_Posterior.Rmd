...
```{r}
  opt <- list()
  opt$datasets <- c("CCLE", "GDSC1000", "CTRPv2", "gCSI","FIMM") # CCLE GDSC GDSC1000 gCSI FIMM CTRPv2
...
Plot median versus median absolute deviation for joined data using the median for drug-type classification.
```{r}
  library(ggrepel)
  for (meas in opt$sensitivity_measure) {
    ...
    for (comp in names(intersection.tibble.list)) {
      ...
      p <- intersection.tibble.list[[comp]] %>%
        filter(measure==meas) %>%
        mutate(all_median = median(value),
               all_mad = stats::mad(value),
               drug=as.factor(drug)) %>% 
        group_by(drug) %>% 
        mutate(median=median(value),
                  ...
                  all_mad=first(all_mad)) %>%
        ungroup() %>%
        mutate(broad = ifelse(median<all_median,'Narrow effect','Broad effect'),
      ...
      ggsave(p, file=file.path(opt$log_dir,meas,comp,paste('Median_v_MAD.pdf',sep='')),
             width = 414, height = 8, units = "in",limitsize=FALSE)
...
```{r}
  ...
  for(num in 1:length(cell_assignments_list)) {
    ...
    iterations_list[[num]] <- accumulate(1:10, iterate_em, .init = list(drug_assignments = drug_assignments,
...
posterior <- posterior_cell_assignments %>%
  ...
  left_join(posterior_drug_assignments %>%
              ...
              rename(drug_type_posterior=posterior,
                     drug_type_likelihood=likelihood,
                     cell_type_prior=p_prior), by=c('drug_type','drug')) %>%
