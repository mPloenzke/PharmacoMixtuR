...
p2_2 <- readRDS('Figs/Fig2/Nilotinib_posterior_CCLE_GDSC.RDS') + 
  ...
  scale_colour_gradient2(midpoint=.975,limits = c(.95,1),mid='grey50',breaks=c(0.95,1),labels=c(0.95,1)) 
...
p2_4$layers[[2]]$data <- p2_4$layers[[2]]$data %>% 
  mutate(beta_curve = ifelse(cell_type == 'sensitive',beta_curve*10, beta_curve)) %>% 
  filter((cell_type=='resistant' & value<.5) | (cell_type=='sensitive' & value>.5))
p2_3$layers[[2]]$data <- p2_3$layers[[2]]$data %>% 
  mutate(beta_curve = ifelse(cell_type == 'sensitive',beta_curve*10, beta_curve)) %>% 
  filter((cell_type=='resistant' & value<.5) | (cell_type=='sensitive' & value>.5))
...
p10 <- readRDS('Figs/Fig4/gdsc/paclitaxel_qq.RDS') + 
  ...
  scale_y_continuous(breaks=c(0,.5,1),labels=c('0','0.5','1'),limits = c(0,1)) + 
  scale_color_brewer(palette = 'Dark2')
p10$layers[[3]]$aes_params$size <- 3


p1$layers[[2]]$data <- p1$layers[[2]]$data %>% 
  mutate(beta_curve = ifelse(cell_type == 'sensitive',beta_curve*10, beta_curve)) %>% 
  filter((cell_type=='resistant' & value<.5) | (cell_type=='sensitive' & value>.5))
p4$layers[[2]]$data <- p4$layers[[2]]$data %>% 
  mutate(beta_curve = ifelse(cell_type == 'sensitive',beta_curve*10, beta_curve)) %>% 
  filter((cell_type=='resistant' & value<.5) | (cell_type=='sensitive' & value>.5))
