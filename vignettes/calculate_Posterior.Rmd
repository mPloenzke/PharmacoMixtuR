...
```{r}
  opt <- list()
  opt$datasets <- c('CCLE','GDSC','FIMM','gCSI') # CCLE GDSC GDSC1000 gCSI FIMM
  ...
  opt$log_dir <- paste(paste(opt$datasets,collapse='_'),'fit_null',sep='_')
  ...
  opt$level.two.empirical <- FALSE # should prior distributions for drug types and individual drugs be fit empirically? 
...
for (comp in names(intersection.fitted.list)) {
  ...
  for (combo in 1:nrow(combos)) {
    ...
    for (dr in drugs) {
      ...
      ggplot(aes_string(x=paste(combos[combo,1],'value',sep='_'),
                        y=paste(combos[combo,2],'value',sep='_'))) + 
      geom_point(aes(color=agreement),alpha=.7,size=2) + 
